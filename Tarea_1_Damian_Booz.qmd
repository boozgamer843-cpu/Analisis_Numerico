...
author: "Damian Booz García Gómez"
format: 
  html:
    grid:
      body-width: 1200px
...
plt.xlabel("x")
plt.ylabel("f(x)")
plt.title("Gráfica ej 1 de f(x) = x^2 - 5x + 6")
plt.grid()
plt.show()
```
...
# Raíces (puedes usar las de bisección si quieres)
r1 = 2
r2 = 3
...
plt.annotate("Raíz 1 (2)", (r1, 0), textcoords="offset points", xytext=(-10,10))
plt.annotate("Raíz 2 (3)", (r2, 0), textcoords="offset points", xytext=(-10,10))

plt.xlabel("x")
plt.ylabel("f(x)")
plt.title("Gráfica de f(x) = x^2 - 5x + 6 con raíces")
plt.grid()
plt.show()
```
...
# Raíces por bisección (las tuyas)
r1_bis, _ = optimize.bisect(f, 0.6, 1.4, full_output=True)
r2_bis, _ = optimize.bisect(f, 1.6, 2.4, full_output=True)
r3_bis, _ = optimize.bisect(f, 2.6, 3.4, full_output=True)
...
# Marcar raíces
plt.scatter(r1_bis, 0)
plt.scatter(r2_bis, 0)
plt.scatter(r3_bis, 0)

# Etiquetas
plt.annotate(f"{r1_bis:.2f}", (r1_bis, 0), textcoords="offset points", xytext=(-10,10))
plt.annotate(f"{r2_bis:.2f}", (r2_bis, 0), textcoords="offset points", xytext=(-10,10))
plt.annotate(f"{r3_bis:.2f}", (r3_bis, 0), textcoords="offset points", xytext=(-10,10))

plt.xlabel("x")
plt.ylabel("f(x)")
plt.title("Gráfica de f(x) = x^3 - 6x^2 + 11x - 6 con raíces")
plt.grid()
plt.show()
```
...
# Raíces por bisección (las tuyas)
r1_bis, _ = optimize.bisect(f, 0.2, 0.8, full_output=True)
r2_bis, _ = optimize.bisect(f, 1.1, 1.7, full_output=True)
...
# Etiquetas
plt.annotate(f"{r1_bis:.6f}", (r1_bis, 0),
             textcoords="offset points", xytext=(-15,10))
plt.annotate(f"{r2_bis:.6f}", (r2_bis, 0),
             textcoords="offset points", xytext=(-15,10))

plt.xlabel("x")
plt.ylabel("f(x)")
plt.title("Gráfica de f(x) = x^3 - 5x^2 e^{-x} + e^{-3x} con raíces")
plt.grid()
plt.show()
```
...
for a in valores_a:
    ...
    # Graficar función
    y = g(x, a)
    linea, = plt.plot(x, y, label=f"a={a}")
    
    # Raíz por bisección
    r_bis, _ = optimize.bisect(
        g,
        0.0,
        1.2,
        args=(a,),
...
plt.xlabel("x")
plt.ylabel("g(x)")
plt.title("g(x) = e^{-x} - cos(ax) con raíces marcadas")
plt.legend()
plt.grid()
plt.show()
```
...
for b in valores_b:
    ...
    # Graficar función
    y = g(x, b)
    linea, = plt.plot(x, y, label=f"b={b}")
...
plt.xlabel("x")
plt.ylabel("g(x)")
plt.title("g(x)=e^x - log(x+1) - b con raíces")
plt.legend()
plt.grid()
plt.show()
```
...
for c in valores_c:
    ...
    # Graficar función
    y = f(x, c)
    linea, = plt.plot(x, y, label=f"c={c}")
    ...
    for (a, b) in intervalos:
        r = optimize.bisect(lambda x: f(x, c), a, b)
...
plt.xlabel("x")
plt.ylabel("f(x)")
plt.title("f(x)=sqrt(x) - sin(x) - c con raíces")
plt.legend()
plt.grid()
plt.show()
```
...
# Resolver sistema
solucion = optimize.fsolve(system, (1, 1))
...
plt.annotate(f"({x_sol:.3f}, {y_sol:.3f})",
             (x_sol, y_sol),
             textcoords="offset points",
             xytext=(10,10))
...
plt.xlabel('x')
plt.ylabel('y')
plt.title('Intersección del sistema no lineal')
plt.grid()
plt.show()
```
...
# Resolver equilibrio
equilibrio = optimize.fsolve(system, (50, 2))
q_eq, p_eq = equilibrio

# Gráfica
plt.figure(figsize=(8,6))

plt.plot(p, q_d, color='red', label='Demanda')
plt.plot(p, q_s, color='blue', label='Oferta')

# Punto de equilibrio
plt.scatter(p_eq, q_eq,
            color='black',
            s=120,
            zorder=5)

# Líneas guía
plt.axvline(p_eq, linestyle='--')
plt.axhline(q_eq, linestyle='--')

# Etiqueta del punto
plt.annotate(f"E({p_eq:.2f}, {q_eq:.2f})",
             (p_eq, q_eq),
             textcoords="offset points",
             xytext=(10,10))
...
plt.xlabel('Precio (p)')
plt.ylabel('Cantidad (q)')
plt.title('Equilibrio de mercado\nDemanda vs Oferta no lineales')
plt.legend()
plt.grid()
plt.show()

print(f"Precio de equilibrio: {p_eq:.4f}")
